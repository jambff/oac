...
export const createResponseDebugInterceptor = (onError?: (error: any) => void) => ({
  ...
  error: (error: any) => {
    ...
    const msg = `${finalStatus} ${data?.message || message} [${method.toUpperCase()} ${url}]`;
    const jambffError = new JamBffError(finalStatus, msg, data?.errors);
    const logError = onError || console.error;

    if (isServerError) {
      logError(jambffError);
    }
